            ...
            if should_flush and batch:
                for event in batch:
                    try:
                        ...
                        if source_ip and event_type in ["ssh_brute_force", "web_brute_force", "cpu_spike", "ram_spike", "syn_flood_inbound", "syn_flood_outbound"]:
                            # Tạo prediction dict cho AlertManager
                            prediction = {
                                "attack_type": "BruteForce" if event_type in ["ssh_brute_force", "web_brute_force"] else event_type,
...
async def receive_agent_logs(
    ...
    for event in payload.events:
        try:
            ...
            # Trigger alert manager for web-related and high-severity events even if the
            ...
            if queue_item.get("event_type") in {
                ...
                "path_traversal",
                "web_brute_force",
            } or queue_item.get("severity") in {"high", "critical"}:
                try:
                    from backend.alert_engine.alert_manager import get_alert_manager
