...
def scan_web_attacks(log_path: str) -> Optional[Dict]:
    ...
    for line in new_lines:
        ...
        if dvwa_login_path.search(path):
            ...
            # DVWA brute-force endpoints thường trả 200 OK trên thất bại, không phải 401/403.
            # Vì vậy chúng ta cũng coi các request đến /vulnerabilities/brute/ là cố gắng brute-force.
            if status in (401, 403) or ("vulnerabilities/brute" in path.lower() and status == 200):
