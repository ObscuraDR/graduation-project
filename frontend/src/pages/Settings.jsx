...
import { Play, Square, Key, Wifi, RefreshCw, Shield, Globe, Ban } from 'lucide-react'
import {
    ...
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            {health ? (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[['PostgreSQL', health.postgres?.connected], ['Redis', health.redis?.connected],
                  ['MongoDB', health.mongo?.connected], ['ML Model', health.model_loaded]].map(([name, ok]) => (
                ...
                <tbody className="divide-y divide-gray-50">
                  {blacklist.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="px-3 py-2 text-xs">{item.country_code || '—'}</td>
      ...
      {tab === 'Geo Block' && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
              <Globe className="w-4 h-4 text-purple-600" /> Geo Blocking — Quick Add
            </h3>
              ...
              <div className="flex flex-wrap gap-2">
                {geoRules.filter(r => r.is_active).map((rule) => (
                  <div key={rule.id} className="flex items-center gap-2 px-3 py-1.5 bg-purple-50 border border-purple-200 rounded-lg">
                    ...
                    <button onClick={() => handleRemoveGeoBlock(rule.country_code)} className="text-purple-400 hover:text-red-500 ml-1">✕</button>
                  </div>
                ))}
              </div>
