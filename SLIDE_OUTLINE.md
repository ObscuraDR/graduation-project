...
6. Validate with a comprehensive automated test suite (58 tests)
...
|---|---|---|
| API Key Auth | `X-API-Key` header, 401 on failure | `test_api_security.py` (8 tests) |
| Rate Limiting | Sliding window per IP: 10/30/60 req per 60s | `test_rate_limiting.py` (11 tests) |
| Input Validation | Pydantic + custom validators; rejects injection | `test_input_validation.py` (39 tests) |
| Alert Suppression | 4-gate chain in AlertManager | `test_alerts.py` (6 tests) |
...
**Speaker notes:**
> "The test suite is fully CI-safe. I can run all 58 tests on any machine without PostgreSQL, Npcap, or an SMTP server. The integration tests build minimal ML artifacts in a temporary directory and use SQLite in-memory for database assertions."
...
**Content:**
- Docker Compose services: `ids-backend`, `postgres`
...
3. Integrate Redis for distributed state management
4. Deploy on Kubernetes for horizontal scaling
5. Supplement CICIDS2017 with newer datasets (CIC-IDS-2018, UNSW-NB15)

**Speaker notes:**
> "In conclusion, I have built a complete, tested, and demo-ready intrusion detection system. The main limitation is Scapy's throughput ceiling, which is acceptable for a lab environment but would need to be addressed for production deployment. Thank you — I am happy to take questions."

---

## Demo Flow Plan (5 Minutes)

Run `scripts/demo_full.ps1` in one terminal and keep a browser open at `http://localhost:8000/docs`.

| Time | Action | Expected Output |
|---|---|---|
| 0:00 | Show `docker-compose ps` | postgres container `healthy` |
| 0:30 | Show `GET /health` in browser or curl | `{"status": "healthy"}` |
...
| 5:00 | Show `pytest backend/tests/ -v` (pre-run result) | `58 passed` |
